...
// ── Auth ──────────────────────────────────────────────────────────────────────
function AuthScreen() {
  const [email,setEmail]=useState(''), [sent,setSent]=useState(false), [loading,setLoading]=useState(false), [err,setErr]=useState('')
  const send = async () => {
    if (!email.trim()) return; setLoading(true); setErr('')
    const {error}=await supabase.auth.signInWithOtp({email:email.trim(),options:{emailRedirectTo:window.location.origin}})
    setLoading(false); if(error){setErr(error.message);return}; setSent(true)
  }
  return (
    <div style={{minHeight:'100dvh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'40px 28px',background:'#070707'}}>
      ...
      {sent?(
        <div className="rise" style={{textAlign:'center',maxWidth:280}}>
          ...
          <div style={{fontSize:11,color:'#555',lineHeight:2}}>Magic link sent to<br/><span style={{color:'#00d4ff'}}>{email}</span><br/>Tap it to sign in.</div>
        </div>
      ):(
        <div className="rise" style={{width:'100%',maxWidth:340}}>
          <div style={{fontSize:9,color:'#444',letterSpacing:4,marginBottom:20}}>SIGN IN OR CREATE ACCOUNT</div>
          <input type="email" autoFocus value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={e=>e.key==='Enter'&&send()} placeholder="your@email.com"
            style={{width:'100%',background:'#0d0d0d',border:'1px solid #222',borderRadius:14,padding:'17px',color:'#fff',fontSize:14,marginBottom:10,transition:'border-color .2s'}}
            onFocus={e=>e.target.style.borderColor='#00d4ff'} onBlur={e=>e.target.style.borderColor='#222'}/>
          {err&&<div style={{fontSize:11,color:'#f87171',marginBottom:10}}>{err}</div>}
          <button onClick={send} style={{width:'100%',borderRadius:16,padding:'20px',background:email.trim()?'#00d4ff':'#111',color:email.trim()?'#000':'#444',fontSize:12,fontWeight:700,letterSpacing:4,boxShadow:email.trim()?'0 10px 40px #00d4ff40':'none',transition:'all .25s',display:'flex',alignItems:'center',justifyContent:'center',gap:10}}>
            {loading?<Spinner color="#000" size={16}/>:'SEND MAGIC LINK →'}
          </button>
          <div style={{fontSize:9,color:'#333',letterSpacing:2,textAlign:'center',marginTop:20}}>NO PASSWORD. NO BS.</div>
        </div>
      )}
    </div>
  )
