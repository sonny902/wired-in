  ...
  return (
    <div style={{minHeight:'100dvh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'40px 28px',background:'#070707'}}>
      <div style={{marginBottom:52,textAlign:'center'}}>
        ...
        <div style={{fontSize:9,color:'#444',letterSpacing:5,marginTop:8}}>ZERO GAP FOCUS</div>
      </div>
      {sent ? (
        ...
        <div className="rise" style={{width:'100%',maxWidth:340}}>
          <div style={{fontSize:9,color:'#555',letterSpacing:4,marginBottom:20}}>SIGN IN OR CREATE ACCOUNT</div>
  ...
  return (
    <div style={{minHeight:'100dvh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'40px 28px',background:'#070707'}}>
      ...
      <div key={step} className="rise" style={{width:'100%',maxWidth:340}}>
        <div style={{fontSize:9,color:'#555',letterSpacing:4,marginBottom:24}}>MODE {mi+1} OF 2 — {isName?'NAME IT':'PICK A COLOUR'}</div>
        {isName ? (
  ...
  return (
    <div style={{margin:'18px 20px 0',background:`linear-gradient(145deg,${c}12 0%,transparent 55%)`,border:`1px solid ${c}28`,borderRadius:22,padding:'24px 22px',position:'relative',overflow:'hidden'}}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
        <div style={{fontSize:8,color:'#555',letterSpacing:4}}>RIGHT NOW</div>
...
function IdleHero({ next, color }) {
  if (!next) return (
    <div style={{margin:'18px 20px 0',background:'#111',border:'1px solid #1e1e1e',borderRadius:22,padding:'40px 22px',textAlign:'center'}}>
      <div style={{fontSize:36,marginBottom:14,opacity:.1}}>🌙</div>
      <div style={{fontSize:9,color:'#444',letterSpacing:4}}>NOTHING SCHEDULED</div>
    </div>
  )
  const mins = Math.max(0, next.start - nowM())
  const c = itemColor(next, color)
  return (
    <div style={{margin:'18px 20px 0',background:'#111',border:'1px solid #1e1e1e',borderRadius:22,padding:'24px 22px'}}>
      <div style={{fontSize:8,color:'#444',letterSpacing:4,marginBottom:12}}>FREE TIME</div>
      <div style={{fontFamily:"'Bebas Neue'",fontSize:28,color:'#555',letterSpacing:1,marginBottom:8}}>
...
function UpNextRow({ item, color, delay=0 }) {
  ...
  return (
    <div className="slide-in" style={{display:'flex',alignItems:'center',gap:14,padding:'13px 0',borderBottom:'1px solid #1a1a1a',animationDelay:`${delay}ms`}}>
      <div style={{width:3,height:38,borderRadius:2,background:c,flexShrink:0,boxShadow:`0 0 8px ${c}60`}}/>
      <div style={{flex:1,minWidth:0}}>
        <div style={{fontSize:12,color:'#bbb',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{item.title}</div>
        <div style={{fontSize:9,color:'#555',marginTop:4,letterSpacing:.5}}>{fmtT(item.start)} · {item.duration}m</div>
      </div>
  ...
  return (
    <div style={{paddingBottom:110}}>
      ...
      <div style={{padding:'58px 20px 0',display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
        <div>
          <div style={{fontSize:9,color:'#444',letterSpacing:5,marginBottom:10}}>WIRED IN TO</div>
          ...
          <div style={{fontSize:9,color:'#555',letterSpacing:1.5,marginTop:8}}>{dayLbl()} · {clockStr()}</div>
        </div>
        <div style={{display:'flex',flexDirection:'column',gap:10,paddingTop:8,alignItems:'flex-end'}}>
          ...
          <button onClick={onSettings} style={{color:'#555',fontSize:22,letterSpacing:3,padding:'0 2px',transition:'color .2s'}}
      ...
      {/* Stats row */}
      <div style={{display:'flex',gap:0,margin:'20px 20px 0',background:'#111',border:'1px solid #1e1e1e',borderRadius:16,overflow:'hidden'}}>
        {[
          ...
          <div key={i} style={{flex:1,padding:'14px 16px',borderRight:i<2?'1px solid #111':'none'}}>
            <div style={{fontSize:8,color:'#888',letterSpacing:2.5,marginBottom:4}}>{s.label}</div>
      ...
      {upcoming.length>0 && (
        <div style={{margin:'22px 20px 0'}}>
          <div style={{fontSize:9,color:'#444',letterSpacing:4,marginBottom:6}}>UP NEXT</div>
      ...
      {/* Day progress */}
      {totalCnt>0 && (
        <div style={{margin:'22px 20px 0',padding:'18px',background:'#111',border:'1px solid #1e1e1e',borderRadius:16}}>
          <div style={{display:'flex',justifyContent:'space-between',marginBottom:10}}>
            <div style={{fontSize:9,color:'#888',letterSpacing:3}}>DAY PROGRESS</div>
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100dvh',paddingBottom:80}}>
      <div style={{padding:'56px 20px 16px',flexShrink:0}}>
        <div style={{fontSize:9,color:'#444',letterSpacing:4,marginBottom:4}}>TIMELINE</div>
        <div style={{fontFamily:"'Bebas Neue'",fontSize:42,color:modeColor,letterSpacing:3}}>{mode.name}</div>
        <div style={{fontSize:9,color:'#555',letterSpacing:1.5,marginTop:4}}>{dayLbl()}</div>
      </div>
          ...
          {hours.map(h=>{
            ...
            return (
              <div key={h} style={{position:'absolute',top,left:-44,right:0,display:'flex',alignItems:'center',gap:10,pointerEvents:'none'}}>
                <div style={{fontSize:9,color:'#666',width:36,textAlign:'right',flexShrink:0,letterSpacing:.5}}>{h%12||12}{h>=12?'p':'a'}</div>
          ...
          {timeline.map((item,i)=>{
            ...
            return (
              <div key={item.id+i} style={{
                ...
                <div style={{fontSize:11,color:isPast?'#444':isCur?'#fff':'#aaa',fontWeight:isCur?500:400,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',letterSpacing:.2}}>{item.title}</div>
...
function PlanItem({ item, color, type, onDelete, onToggle }) {
  ...
  return (
    <div style={{display:'flex',alignItems:'center',gap:12,padding:'13px 0',borderBottom:'1px solid #1a1a1a',animation:'rise .35s both'}}>
      {onToggle && (
      ...
      <div style={{flex:1,minWidth:0}}>
        <div style={{fontSize:12,color:item.done?'#333':'#ccc',textDecoration:item.done?'line-through':'none',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{item.title}</div>
        <div style={{fontSize:9,color:'#555',marginTop:4,letterSpacing:.5}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:8}}>
        ...
        <button onClick={()=>onDelete(item.id)} style={{color:'#555',fontSize:18,padding:'0 2px',transition:'color .2s'}} onMouseEnter={e=>e.target.style.color='#555'} onMouseLeave={e=>e.target.style.color='#1e1e1e'}>×</button>
      </div>
    </div>
  )
}
    ...
    <div style={{marginBottom:4}}>
      <div onClick={()=>setOpen(p=>!p)} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'16px 20px',cursor:'pointer'}}>
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          ...
          <div style={{fontSize:9,color:'#999',letterSpacing:3.5}}>{title}</div>
          <div style={{fontSize:9,color:'#555',background:'#111',padding:'2px 8px',borderRadius:20}}>{items.length}</div>
        </div>
        <div style={{display:'flex',alignItems:'center',gap:12}}>
          ...
          <div style={{color:'#555',fontSize:12,transition:'transform .2s',transform:open?'rotate(0)':'rotate(-90deg)'}}>▾</div>
        </div>
      </div>
      {open&&(
        <div style={{padding:'0 20px',paddingBottom:items.length?8:0}}>
          {items.length===0?(
            <div style={{fontSize:9,color:'#333',letterSpacing:3,padding:'16px 0',textAlign:'center'}}>{empty}</div>
          ):items.map(item=>(
  ...
  return (
    <div style={{paddingBottom:110}}>
      <div style={{padding:'58px 20px 20px'}}>
        <div style={{fontSize:9,color:'#444',letterSpacing:4,marginBottom:6}}>PLAN YOUR DAY</div>
...
function AddFixedSheet({ color, modeName, onAdd, onClose }) {
  ...
  return (
    <Sheet onClose={onClose}>
      ...
      <div style={{fontSize:9,color:'#555',letterSpacing:3,marginBottom:10}}>START TIME</div>
      ...
      <div style={{fontSize:9,color:'#555',letterSpacing:3,marginBottom:10}}>DURATION</div>
...
function AddTaskSheet({ color, modeName, onAdd, onClose }) {
  ...
  return (
    <Sheet onClose={onClose}>
      ...
      <div style={{fontSize:9,color:'#555',letterSpacing:3,marginBottom:10}}>DURATION</div>
      <DurPicker val={dur} onChange={setDur} color={color}/>
      <div style={{fontSize:9,color:'#555',letterSpacing:3,marginBottom:10}}>PRIORITY</div>
...
function AddQWSheet({ color, onAdd, onClose }) {
  ...
  return (
    <Sheet onClose={onClose}>
      ...
      <div style={{fontSize:11,color:'#555',marginBottom:18,lineHeight:1.8}}>A ~10 minute task. Wired In fills spare gaps in your day with these automatically.</div>
    ...
    <div style={{position:'fixed',bottom:0,left:0,right:0,zIndex:100,background:'rgba(5,5,5,.96)',backdropFilter:'blur(24px)',borderTop:'1px solid #0f0f0f',padding:'10px 0 28px',display:'flex'}}>
      {items.map(it=>(
        <button key={it.key} onClick={()=>setTab(it.key)} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:5,padding:'8px 0'}}>
          ...
          <div style={{fontSize:8,letterSpacing:2,color:tab===it.key?color:'#555',transition:'color .2s'}}>{it.label}</div>
      ...
      <div style={{minHeight:'100dvh',background:'#070707',overflowX:'hidden'}}>
        ...
        {syncing&&(
          <div style={{position:'fixed',top:16,right:16,zIndex:300,display:'flex',alignItems:'center',gap:6}}>
            ...
            <span style={{fontSize:8,color:'#555',letterSpacing:2}}>SYNCING</span>
          </div>
        )}
